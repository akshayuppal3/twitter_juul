...
def run_lstm(train_data, test_data, Y_train, Y_test, dimension, epoch, cross_splits=5,weight=None):
	scores = []
	...
	X = lstm.get_encoded_data(final_data, keras_tkzr, max_len)
...
def run_lstm_comb(train_data, test_data, Y_train, Y_test, dimension, epoch, weight=None):
	scores = []
	## print winodow , max_len for analysis purpose
	max_len = util.get_max_length(train_data)
	if max_len > 60:
